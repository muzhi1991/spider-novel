...
__GLOBAL_EXECUTOR__ = ThreadPoolExecutor(max_workers=1500)
...
class SpiderDetailTask(SpiderTask):
    ...
    async def start(self, proxy):
        ...
        if book_info is None:
            try:
                ...
                logger.exception(
                    "consumer {} - task {}: SpiderDetailTask -- 爬图书详情错误！！！！ book_url:{}".format(
                        self.consumer_id,
                        self.id, book_url))
            if book_info is None:
                # 代理问题切换爬虫重试
            ...
            try:
                ...
                await SpiderTask.loop.run_in_executor(__GLOBAL_PROCESS_EXECUTOR__,
                    SpiderDetailTask.add_to_book_manifest_detail_file, book_info)
    ...
    async def spider_detail(self, session, book_url, spider_name, proxy):
        ...
        while try_num <= 3 and infos is None:
            ...
            try:
                infos =await SpiderTask.loop.run_in_executor(__GLOBAL_PROCESS_EXECUTOR__,
                    __spider_list__[spider_name].spider_parse_detail, book_url, resp.content)
